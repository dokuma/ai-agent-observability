...
class MetricsAgent:
    ...
    def _get_context_aware_messages(self, state: AgentState) -> list[BaseMessage]:
        ...
        return build_context_aware_messages(
            ...
            agent_identifier="Metrics Agent",
    ...
    def _build_env_search_query(plan: Any) -> str:
        """調査計画からメトリクス環境情報検索用クエリを構築.

        PromQLクエリからメトリクス名とラベル値を抽出し、
        ContextStoreにインデックスされた環境情報から
        関連するdatasource・メトリクス・jobの情報を取得する。
        """
        terms: list[str] = []
        if plan:
        ...
        return " ".join(terms) if terms else "prometheus metrics datasource"
